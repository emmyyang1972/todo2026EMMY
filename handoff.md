...
## 2026-09-17 醫院永續報告書學習模組

### 已完成

- 新增 `/esg` 學習模組，包含「每日條文拆解」與「醫院永續報告書 ↔ GRI 對照」兩個頁籤。
- 建立 `lib/esg-learning.ts`，先納入 GRI 1、GRI 2、GRI 3、GRI 308-1／308-2 與醫院永續報告書案例的 11 張學習卡。
- 每日卡片依日期輪替，可上一條／下一條、查看白話拆解、醫院落地方式、應備證據與反思題，並可標記完成。
- 完成學習進度先保存於瀏覽器 localStorage（`focusdesk.esg.completed`），重新整理後仍保留。
- 對照表先涵蓋組織邊界、治理、利害關係人、重大主題、病安、能源／氣候、廢棄物、供應鏈與職安。
- 來源指向 `C:\1ESG` 原始 PDF／Excel 與醫院報告書；沒有複製、搬移或改名原始檔。
- 今日與文獻研究頁面的側邊欄已加入 ESG 學習入口。

### 驗證

- `npx.cmd tsc --noEmit`：通過。
- `npm.cmd run build`：受限環境外通過，新增 `/esg` route 已產生。

### 下一步

1. 逐份讀取 `C:\1ESG` 的 GRI 與醫院報告書，擴充到完整條文索引。
2. 若要跨裝置保存學習進度，再將完成狀態接到 Supabase。
3. 確認內容後部署 Vercel。
