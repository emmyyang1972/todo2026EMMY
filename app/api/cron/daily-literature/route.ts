...
import { collectDailyDigest } from '../../../../lib/daily-literature'
...
export async function GET(request: Request) {
  ...
  if (!url || !serviceKey || !ownerId) return NextResponse.json({ error: '缺少 Supabase 或 LITERATURE_OWNER_ID 環境變數' }, { status: 503 })
  ...
  const { data: digestRow, error: digestError } = await supabase.from('literature_digests').upsert({ owner_id: ownerId, digest_date: date, paper_count: digest.papers.length, generated_at: new Date().toISOString() }, { onConflict: 'owner_id,digest_date' }).select('id').single()
  ...
  return NextResponse.json({ ok: true, date, papers: digest.papers.length, ideas: digest.ideas.length, emailSent: false })
}
